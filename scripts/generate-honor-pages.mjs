...
<link rel="stylesheet" href="/styles/detail-pages.css"></head>
<body><main class="page"><nav class="detail-topbar" aria-label="Portfolio navigation"><a class="back" href="/about">← Back to portfolio</a><span class="detail-author">Adriel Magalona</span></nav><header class="page-heading"><p class="eyebrow">Achievement evidence · ${escapeHtml(honor.year)}</p><h1>${escapeHtml(honor.title)}</h1><div class="facts">${honor.organizer ? `<span>${escapeHtml(honor.organizer)}</span>` : ''}${honor.date ? `<span>${escapeHtml(honor.date)}</span>` : ''}${honor.location ? `<span>${escapeHtml(honor.location)}</span>` : ''}</div></header><article class="record"><h2>Portfolio record</h2><p>${escapeHtml(description)}</p>${gallery}${links}</article></main></body></html>`;
}
...
const urls = [
  ['', '1.0'], ['/about', '0.8'], ['/stack', '0.8'], ['/background', '0.7'], ['/projects', '0.9'], ['/contact', '0.6'], ['/gear', '0.6'], ['/destinations', '0.5'],
